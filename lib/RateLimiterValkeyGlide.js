...
const DEFAULT_LIBRARY_NAME = 'ratelimiter';
...
class RateLimiterValkeyGlide extends RateLimiterStoreAbstract {
   ...
   * @param {string} [opts.functionLibName] - Custom name for the function library in Valkey
  ...
  constructor(opts) {
    ...
    this.libraryName = opts.functionLibName || DEFAULT_LIBRARY_NAME;
  }
  ...
  async _loadScripts() {
    ...
    if (!this._scriptLoaded) {
      const script = Buffer.from(`#!lua name=${this.libraryName}
    ...
    } else promises.push(Promise.resolve(this.libraryName));
    ...
    this._scriptLoaded = results[0] === this.libraryName;
  ...
  async close() {
    if (this._scriptLoaded) {
      await this.client.functionDelete(this.libraryName);
      this._scriptLoaded = false;
    }
    if (this._getScriptLoaded) {
