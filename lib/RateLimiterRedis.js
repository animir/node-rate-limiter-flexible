...
class RateLimiterRedis extends RateLimiterStoreAbstract {
  ...
  constructor(opts) {
    ...
    this._delayMultiplierByMaxPointsEnabled = !!opts.delayMultiplierByMaxPointsEnabled;
    ...
    if (secDuration > 0) {
      if(!this.useRedisPackage && !this.useRedis3AndLowerPackage){
        return this.client.rlflxIncr(
          [rlKey].concat([String(points), String(secDuration), String(this._delayMultiplierByMaxPointsEnabled?this.points:0)]));
      }
      if (this.useRedis3AndLowerPackage) {
        return new Promise((resolve, reject) => {
          ...
          if (typeof this.client.rlflxIncr === 'function') {
            this.client.rlflxIncr(rlKey, points, secDuration, this._delayMultiplierByMaxPointsEnabled?this.points:0, incrCallback);
          } else {
            this.client.eval(incrTtlLuaScript, 1, rlKey, points, secDuration, this._delayMultiplierByMaxPointsEnabled?this.points:0, incrCallback);
          }
        });
      } else {
        return this.client.eval(incrTtlLuaScript, {
          keys: [rlKey],
          arguments: [String(points), String(secDuration), String(this._delayMultiplierByMaxPointsEnabled?this.points:0)],
        });
      }
    } else {
      if(!this.useRedisPackage && !this.useRedis3AndLowerPackage){
