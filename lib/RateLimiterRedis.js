...
class RateLimiterRedis extends RateLimiterStoreAbstract {
  ...
  _isRedisReady(rlKey, isReadonly) {
    ...
    if (typeof this.client.isReady === 'boolean' && this.client.isReady !== true) {
      return false;
    }
    // node-redis v4 and above cluster client
    if (this.client._slots && typeof this.client._slots.getClient === 'function') {
      // If the client's socket is not open yet, the slots have not been initialized
      if (typeof this.client.isOpen === 'boolean' && this.client.isOpen !== true) {
        return false;
      }
      if (this.client._slots.getClient(rlKey, isReadonly).isReady !== true) {
        return false;
      }
    }
    return true;
