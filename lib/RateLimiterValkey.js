...
const incrTtlLuaScript = `server.call('set', KEYS[1], 0, 'EX', ARGV[2], 'NX')
...
class RateLimiterValkey extends RateLimiterStoreAbstract {
  ...
  _upsert(rlKey, points, msDuration, forceExpire = false) {
    ...
    if (secDuration > 0) {
      return this.client.rlflxIncr([rlKey].concat([String(points), String(secDuration), String(this.points), String(this.duration)]));
    }
