...
module.exports = class MemoryStorage {
  ...
  async incrby(key, value, durationSec) {
    ...
    if (record) {
      ...
      if (!record.expiresAt || msBeforeExpires > 0) {
        ...
        if (typeof this.onUpsert === 'function') {
          await this.onUpsert(key, { value: record.value, expiresAt: record.expiresAt });
        }
  ...
  async set(key, value, durationSec) {
  ...
  _restoreRecord(key, value, expiresAt) {
    ...
    const durationMs = expiresAt ? expiresAt - now : 0;
