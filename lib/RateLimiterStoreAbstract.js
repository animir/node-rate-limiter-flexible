  ...
  consume(key, pointsToConsume = 1, options = {}) {
    return new Promise((resolve, reject) => {
      ...
      if (this.timeoutMs && this.timeoutMs > 0) {
        const upsertPromise = this._upsert(rlKey, pointsToConsume, this._getKeySecDuration(options) * 1000, false, options)
          .then((res) => {
        ...
        const timeoutPromise = new Promise((_, timeoutReject) => {
          setTimeout(() => {
            timeoutReject(new Error(`Rate limiter operation timed out after ${this.timeoutMs}ms`));
          }, this.timeoutMs);
        });

        Promise.race([upsertPromise, timeoutPromise])
          .catch((err) => {
            reject(err);
          });
      } else {
