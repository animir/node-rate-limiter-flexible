...
module.exports = class RLWrapperBlackAndWhite extends RateLimiterCompatibleAbstract {
  ...
  consume(key, pointsToConsume = 1) {
    return this._execAction('consume', key, [pointsToConsume], true);
  }

  block(key, secDuration) {
    return this._execAction('block', key, [secDuration]);
  }

  penalty(key, points) {
    return this._execAction('penalty', key, [points]);
  }

  reward(key, points) {
    return this._execAction('reward', key, [points]);
  }
  ...
  get(key) {
    ...
    return this._execAction('get', key, []);
  }

  set(key, points, secDuration) {
    return this._execAction('set', key, [points, secDuration]);
  }
