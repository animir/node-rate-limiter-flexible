    ...
    it('use keyPrefix from options', (done) => {
      ...
      worker.on('message', (msg) => {
        if (msg && msg.channel === 'mocha' && msg.test === 'use keyPrefix from options') {
          expect(typeof rateLimiterClusterMaster._rateLimiters[keyPrefix]._memoryStorage._storage['test:use keyPrefix from options'] !== 'undefined')
            .to.equal(true);
          done();
        }
      });
    ...
    if (msg && msg.channel === 'mocha' && typeof msg.test === 'string') {
      switch (msg.test) {
        ...
        case 'create 2 rate limiters depending on keyPrefix':
          const rateLimiterClusterWorker1 = new RateLimiterCluster({ keyPrefix: msg.data[0] });
          const rateLimiterClusterWorker2 = new RateLimiterCluster({ keyPrefix: msg.data[1] });
