...
class RateLimiterPostgres extends RateLimiterStoreAbstract {
  ...
  _getSequelizeConnectionManager() {
    ...
    // Preserve the original error context when no connection manager is available
    const errorMessage = accessError
      ? `Sequelize connection manager is not available. Original error: ${accessError.message}`
      : 'Sequelize connection manager is not available';
