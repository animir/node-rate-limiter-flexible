...
describe('RateLimiterMemory Persistence', function RateLimiterMemoryPersistenceTest() {
  ...
  it('dump results in object with version 1 and all keys', (done) => {
    const rateLimiter = new RateLimiterMemory({ points: 2, duration: 5 });
    rateLimiter.consume('test')
      .then(() => {
        const dump = rateLimiter.dump();
        expect(typeof dump).to.equal('object');
        const data = dump;
  ...
  it('restore correctly from object', (done) => {
    const rateLimiter = new RateLimiterMemory({ points: 2, duration: 5 });
    const dump = {
      version: 1,
      dumpedAt: Date.now(),
  ...
  it('restore correctly when dumpedAt property is omitted', (done) => {
    const rateLimiter = new RateLimiterMemory({ points: 2, duration: 5 });
    const dump = {
      version: 1,
      storage: [{ key: 'test', value: 1, expiresAt: Date.now() + 5000 }]
  ...
  it('TTL arithmetic across simulated downtime (shorter than TTL)', (done) => {
    ...
    const dump = {
      version: 1,
      // dumpedAt is omitted because it is not required for TTL arithmetic
  ...
  it('corrupt input does not throw', () => {
    ...
    expect(() => rateLimiter.restore('invalid json')).to.not.throw();
    expect(() => rateLimiter.restore(null)).to.not.throw();
    expect(() => rateLimiter.restore(undefined)).to.not.throw();
    expect(() => rateLimiter.restore(JSON.stringify({ version: 1, storage: 'not an array' }))).to.not.throw();
  });
  ...
  it('version mismatch does not throw and ignores data', (done) => {
    ...
    expect(() => rateLimiter.restore(dump)).to.not.throw();
    rateLimiter.get('test')
      .then((res) => {
